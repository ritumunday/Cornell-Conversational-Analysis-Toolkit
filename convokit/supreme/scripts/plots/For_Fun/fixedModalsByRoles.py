...
def plotyears(modalnames, bucket, verb):
    ...
    linearr = getfilesarr()
    filtered = {"Justice": {} , "Others" :{}}
    baseline = {"Justice": {} , "Others" :{}}

    # filter lines
    for l in linearr:
        year = int(l.get("Year"))
        if l.get("Role") == "J":
            baseline["Justice"][year] = 1 if baseline["Justice"].get(year) is None else baseline["Justice"][year] + 1
        else:
        ...
        for mod in modalnames:
            if l.get("Mod").lower() == mod.lower():
                if l.get("Role") == "J":
                    filtered["Justice"][year] = 1 if filtered["Justice"].get(year) is None else filtered["Justice"][year] + 1
                else:
...
def dolines(alldict):
    ...
    ax.set_ylabel('use of modal by role over use of all modals by role' )
    ax.set_title(title)
    ax.legend()
...
bucket = 4
modals = ["can" ]
title = 'Can - FixedModalsByRole.py'
filepath = '../results/' + "3-OnlyCanByRole.png"
